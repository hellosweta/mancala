class Board
  ...
  def valid_move?(start_pos)
    raise "Invalid starting cup" unless (0..12).include?(start_pos) && !@cups[start_pos].empty?
  end
  ...
  def make_move(start_pos, current_player_name)
    ...
    until stones.empty?
      ...
      if cup_idx == 6
        @cups[6] << stones.pop if current_player_name == @name_one
      elsif cup_idx == 13
        @cups[13] << stones.pop if current_player_name == @name_two
      else
  ...
  def winner
    ...
    if player1_count == player2_count
      ...
      player1_count > player2_count ? @name_one : @name2_one
    end
  end
end
